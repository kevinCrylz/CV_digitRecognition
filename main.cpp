...
int main(int argc, char* argv[]) {
	...
	const string temp_img_path = "{YOUR_DIR_TO_TEMP_PNG}";
	
	if (argc == 2) {
		if (string(argv[1]) == "temp") {
			run = false;
		}
	}

	if (!run || debug) {
		frame = imread(temp_img_path, IMREAD_COLOR);
		frame.convertTo(frame, -1, 1, -50);
		
		if (argc == 3) {
			if (string(argv[2]) == "0") imgproc(frame);
			else if (string(argv[2]) == "1") redDetect(frame);
		} else {
			redDetect(frame);
		}
	...
	while (!cap.open(i) && i <= 4) { 
		cout << "Video input port " << i++ << " is not available" << endl;
	}

	while (cap.open(i) && run && !debug) {
		cout << "\nVideo input port " << i << " is Selected" << endl;
		...
		if (argc == 2) {
			if (string(argv[1]) == "test") testHSV(frame);
			else if (string(argv[1]) == "red") redDetect(frame);
		} else {
			redDetect(frame);
		}
		waitKey(10);
...
void recognize(Mat recog) {
	...
	int scale = 4;
	erode(recog, recog, getStructuringElement(0, Size(scale, scale), Point(-1, -1)));
	dilate(recog,recog,getStructuringElement(0,Size(scale-2, scale-2),Point(-1,-1)));

	blur(recog, recog, Size(3,3), Point(-1,-1));
	threshold(recog, recog, 10, 255, 0);
